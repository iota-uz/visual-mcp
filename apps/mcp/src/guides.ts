...
Use one stable ref, preferably \`workspace-slug/canvas-slug\`. A canvas id, returned URL, public slug, canvas:// URI, or complete element ref is also accepted. Keep file paths in \`path\`, never in the ref. Start existing work with \`canvas_get\`; its default response is compact. Passing page_id with include:["doc"] returns that Page rather than the complete CanvasFile. Omit page_id only when a complete multi-page file is intentional. A doc projection narrows collections and nodes further, pagination returns a complete next request, \`canvas_file_search\` locates source text, and \`canvas_file_get\` reads up to 20 exact file projections in one call. Set response_mode:"full" only when theme, author, thumbnail, or embed metadata is needed.
...
\`canvas_save\` upserts atomically by ref. A CanvasFile v3 owns ordered Pages of CanvasDoc v2 plus one prototype. Files, assets, checkpoints, visibility, and sharing remain canvas-level. Use \`canvas_edit\` for an ordered atomic batch of exact source replacements and \`canvas_apply_patch\` for Codex-style source patches. Use \`canvas_patch\` when one structural change spans Page title/subtitle/order, CanvasDoc entities, or prototype references; it validates the final CanvasFile once, so replacing a start node needs no temporary intermediate write. Pass canvas.state unchanged as base. The compact response returns the next state for another patch. Single-purpose page/prototype/node tools remain useful for one isolated gesture. On conflict, reread; hash-backed file edits may safely rebase when their targets are unchanged.
...
Search reusable media with \`asset_list\`, inspect with \`asset_get(include_preview=true)\`, and attach an existing immutable revision with \`asset_ref\` at an \`/assets\` path. Use \`asset_upload_url\`/\`asset_finalize\` only to add media directly to a library without a canvas, and \`asset_import\` for HTTPS sources. Moving an asset changes library scope without rewriting existing pinned bindings; deleting archives it. SVG is trusted internal workspace content and must be preserved byte-for-byte. Audio is supported as an immutable private asset with explicit MIME, size and measured metadata checks; pin an exact revision before placing it on a voice, music or SFX timeline track.
