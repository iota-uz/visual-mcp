...
export const providerDefinitions: Definition[] = [
  ...
  ),
  {
    name: "job_reconcile",
    description:
      "Resume recovery of a known provider/persistence receipt without another paid generation POST. Only recovery.kind=reconcile jobs qualify. If job_get instead returns recovery.kind=regenerate with safeToRegenerate=true, a local render/media operation was never applied or its stored bytes are proven unavailable: submit that operation with a new idempotency key instead. Poll job_get after admission.",
    readOnly: false,
