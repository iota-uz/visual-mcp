...
| `canvas_get` | Reads one canvas: metadata and URLs always, plus cursor-paginated `files` / `artifacts` / `versions` / `renders`, `doc`, or `storage`. `doc_projection` implicitly includes `doc`; each unfinished facet returns a ready-to-call `next_request` pinned to the current version. Bytes come back as links, never inlined. |
| `canvas_file_get` | Reads up to 20 canvas files in one bounded call with version/hash metadata. Full/line reads are UTF-8; exact byte ranges are base64 and declare their `encoding`. |
| `canvas_file_search` | Searches current UTF-8 canvas sources for a literal string and returns bounded line matches before a focused read or edit. |
| `canvas_snapshot` | Captures a whole native canvas, one `ref_id` node, or an exact world-coordinate region. For iframe/image nodes, `clip: "content"` captures only the inner viewport without device/browser chrome; `clip: "frame"` remains the default. It retries transient readiness once and reports resource diagnostics. Use `response_mode: "link"` to avoid returning image bytes; PNGs above 5 MB do this automatically. Public embed metadata remains in the separate `canvas_embed` operation. |
| `canvas_edit` | Atomically applies up to 50 exact `old_string` → `new_string` edits across UTF-8 files with version/hash conflict protection. |
...
For a website rather than an app screen, use `frame: { kind: "device", preset: "iphone-safari" }` or `preset: "desktop-safari"` and omit `viewport` — each preset carries its own (284×590 and 1280×800). The renderer draws the iPhone body with the iOS status bar and Safari controls, or a Safari window with its traffic lights and address bar; `frame.url` fills that address bar and `frame.display: "full-height"` grows the shell so a whole page is visible instead of being cropped at the screen edge.
